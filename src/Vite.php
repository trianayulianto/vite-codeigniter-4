...
namespace TYlnt\Vite;

use Closure;
use Exception;
use TYlnt\Vite\Support\Htmlable;
use TYlnt\Vite\Support\HtmlString;
...
class Vite implements Htmlable
{
    ...
    public function useCspNonce($nonce = null)
    {
        ...
        return $this->nonce = $nonce ?? random_string('alnum', 40);
    }
    ...
    {
        return $this->hotFile ?? FCPATH.'hot';
    }
     ...
     * @return \Vite\Support\HtmlString
     *
     * @throws \Exception
     */
    public function __invoke($entrypoints, $buildDirectory = null)
    {
        ...
        if ($this->isRunningHot()) {
            ...
            return new HtmlString(implode('', $entrypoints));
        }
        ...
        foreach ($entrypoints as $entrypoint) {
            ...
            foreach ($chunk['imports'] ?? [] as $import) {
                foreach ($manifest[$import]['css'] ?? [] as $css) {
                    $partialManifest = array_filter($manifest, fn ($item) => isset($item['file']) && $item['file'] == $css);

                    array_push($tags, $this->makeTagForChunk(
        ...
        [$stylesheets, $scripts] = (function (array $elements, Closure $callback) {
            $matches = $noMatches = [];
            ...
            $tags,
            fn ($tag) => str_starts_with($tag, '<link')
        );

        return new HtmlString(implode('', $stylesheets).implode('', $scripts));
    }
    ...
    {
        $attributes = array_filter($attributes, fn ($value) => ! in_array($value, [false, null], true));

        array_walk($attributes, fn ($value, $key) => $value === true ? [$key] : [$key => $value]);

        $attributes = array_map(fn ($value, $key) => is_int($key) ? $value : $key.'="'.$value.'"', array_values($attributes), array_keys($attributes));
     ...
     *
     * @return \Vite\Support\HtmlString|void
     */
    ...
    {
        return FCPATH.$buildDirectory.'/manifest.json';
    }
